...
class LogisticRegression:
    ...
    def computeGradient(self, theta, X, y, regLambda):
        ...
        gradient_0 = self.alpha*np.ones((1,n))*(yhat-y)    # compute gradient 0

        # compute the remain gradient
        if self.regNorm == 2:
            gradient_remain = self.alpha(X.T*(yhat-y)+regLambda*theta)
        elif self.regNorm == 1:
            gradient_remain = self.alpha(X.T*(yhat-y)+regLambda)
...
class LogisticRegressionAdagrad:
    ...
    def computeGradient(self, theta, X, y, regLambda):
        ...
        gradient_0 = X.T*(yhat-y)    # matrix
        # self.G[0] +=((yhat[0,0]-y[0,0])*X[0,0])**2    # compute G[0]
        ...
        if self.regNorm == 2:
            ...
            gradient_remain = X.T*(yhat-y)+regLambda*np.ones((d,1))

        # print(gradient_remain)
        
        gradient = np.r_[gradient_0[0],gradient_remain[1:]]
    ...
    def fit(self, X, y):
        ...
        for k in range(self.maxNumIters):
            for i in range(10):
                ...
                alpha = self.alpha/(np.sqrt(self.G)+self.ksi*np.ones(d))    # compute alpha, a row
